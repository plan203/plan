...
const Friends: React.FC = () => {
  ...
  const handleSendFriendRequest = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error instanceof Error) {
        // Remove redundant "Error sending friend request:" prefix if present
        const errorMessage = error.message.startsWith('Error sending friend request:') 
          ? error.message.replace('Error sending friend request:', '').trim()
          : error.message;
  ...
  const handleRemoveFriend = async (friendId: string, friendName: string) => {
    ...
    const confirmed = window.confirm(
      `Are you sure you want to remove ${friendName} from your friends?\n\nThis action cannot be undone and will remove the friendship from both accounts.`
    );
    ...
    if (success) {
      // Show success popup
      alert(`${friendName} has been successfully removed from your friends list.`);
    } else {
